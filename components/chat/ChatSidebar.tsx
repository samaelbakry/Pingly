...
export default function ChatSidebar({
  ...
  return (
    <div className="relative flex h-full flex-col rounded-3xl border border-white/80 bg-white/70 p-4 shadow-xl shadow-orange-500/5 backdrop-blur-2xl">
      <div className="flex items-center justify-between border-b border-orange-100/60 pb-3.5">
        <div>
          <h2 className="flex items-center gap-1.5 text-sm font-black tracking-tight text-slate-800">
            Messages
      ...
      <div className="relative my-3">
        ...
        <Input
          ...
          className="h-10 rounded-2xl border-slate-200/80 bg-slate-50/70 pl-9 pr-4 text-xs transition-all placeholder:text-slate-400 focus-visible:border-orange-400 focus-visible:bg-white focus-visible:ring-4 focus-visible:ring-orange-500/10"
        />
      </div>
            ...
            return (
              <button
                ...
                className={`group relative flex w-full cursor-pointer items-center gap-3 rounded-2xl border p-3 text-left transition-all duration-200 ${
                  isSelected
                    ? "border-orange-300/50 bg-linear-to-r from-orange-500/15 to-rose-500/10 shadow-xs"
                    : "border-transparent hover:bg-slate-50/80"
                }`}
              >
                {isSelected && (
                  <div className="absolute left-0 top-1/2 h-7 w-1 -translate-y-1/2 rounded-r-full bg-linear-to-b from-orange-500 to-rose-500" />
                )}

                <Avatar className="h-11 w-11 border border-slate-200/60 shadow-xs">
      ...
      <div className="mt-2 border-t border-orange-100/60 pt-3">
        <Button
          type="button"
          onClick={() => setIsNewChatOpen(true)}
          className="flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-linear-to-r from-amber-500 via-orange-500 to-red-500 text-xs font-bold text-white shadow-lg shadow-orange-500/20 transition-all hover:opacity-95 hover:shadow-orange-500/30 active:scale-[0.99]"
        >
